...
async def add_balance_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Команда для разработчика - пополнить баланс"""
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("withdraw_"):
        ...
        if result:
            ...
            final_price = item_price * 1.2 + random.randint(1, 55) / 100
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # === ВЫВОД СКИНА (ВВОД ПАТТЕРНА) - СКИН УДАЛЯЕТСЯ ИЗ ИНВЕНТАРЯ ===
    if user_id in waiting_for_pattern:
        ...
        if not result:
            conn.close()
            await update.message.reply_text(
                "❌ Ошибка: предмет не найден или уже выведен.",
                reply_markup=main_menu(user_id)
            )
            del waiting_for_pattern[user_id]
            return
        ...
        # ПОМЕЧАЕМ СКИН КАК ПРОДАННЫЙ (УДАЛЯЕМ ИЗ ИНВЕНТАРЯ)
        c.execute("UPDATE inventory SET sold = 1 WHERE id = ?", (item_id,))
        conn.commit()
        conn.close()
        
        # Цена = цена_скина × 1.2 + рандомные копейки от 0.01 до 0.55
        final_price = item_price * 1.2
        random_cents = random.randint(1, 55) / 100
        final_price = final_price + random_cents
        final_price = round(final_price, 2)
        ...
        try:
            ...
            await update.message.reply_text(
                ...
                f"📦 **Скин удалён из твоего инвентаря!**",
                reply_markup=main_menu(user_id),
                parse_mode="Markdown"
            )
